...
genehancer_data <- read.table('../raw_data/enhancer_lengths_new.csv', sep='\t', header=1)
...
library('igraph')
string_data <- read.table('../raw_data/string_db_filtered.txt', sep='\t', header=1) #filtered means, all PPIs with a combined score >700, i.e. "high-confidence" of interaction according to STRING DB, previously filtered using python
string_data[,1] <- gsub('9606.','', string_data[,1]) # the genes all have a preceeding "9606." ==> cut it out
...
median_tpm_per_tissue_data <- read.table('output_files/median_gene_tpm_per_tissue.tsv', sep='\t', header=T, check.names=F) 

median_gene_tpm_data <- read.table('output_files/median_gene_tpm.tsv', sep='\t', header=T, check.names=F)


save(file='Rdata/datacollection.Rdata', list=ls())
