...
load(file='Rdata/visualization.Rdata') #the most important things generated in this script are in this Rdata


df1 <- read.table('output_files/raw_final_df.txt',sep='\t',header=1)
df <- df1
...
df$rowsums <- rowSums(!is.na(df[,c(13:ncol(df))]))
...
write.table(for_plot,file='output_files/for_plot.tsv',sep='\t',row.names = FALSE)
...
x <- list( vg_AE = vg_aes,  loeuf_score = loeufs, ncGERP = ncgerps,ncRVIS=ncrviss, RVIS = rviss, median_TPM = tpms)
#loeuf_score = loeufs, ncGERP = ncgerps, RVIS = rviss, vg_AE = vg_aes, vg_eQTL = vg_eqtls, STRING = strings,vg_eQTL = vg_eqtls, TPM = tpms
ggv <- ggVennDiagram(x, label='count')
ggv
...
save(file='Rdata/visualization.Rdata',list=ls())
