...
library('ggcorrplot')
library('gridExtra')
load(file = 'Rdata/gtex_transcript_length_analysis.Rdata') #this file contains the most important stuff generated in this script
...
transcript_length <- read.table('output_files/ATLPT_new_abbrv.tsv',sep='\t', header=TRUE) #generated w python on UPPMAX
...
##NOW LET'S ANALYSE THE CORRELATION OF TRANSCRIPT LENGTH AND VG WITHIN ONE GENE OVER MULTIPLE TISSUES, i.e. "row-wise" correlation of the fields of VG_AE per tissue and transcript length per tissue
#PROBLEM ==> FOR MANY GENES ONLY A FEW TISSUES AVAILABLE ==> might filter those few tissues(chose 10 tissues ==> correlation of two tissues pretty weak) tissue available out
rowsums <- rowSums(!is.na(avg_vg_ae_data))
hist(rowsums, breaks = 50) #chose 10 tissues, although there are a lot of genes with less than 10 representative tissues
...
#takes a couple of minutes, as 7877 correlations are calculated
...
new_df <- as.data.frame(corr_list, row.names = 'corr_transcript_length')
new_df <- as.data.frame(t(new_df))
#new_df[sort(abs(new_df$transcript_length))]
nums <- c(1:nrow(new_df))
new_df$genes <- nums
group <- rep.int(1, nrow(new_df)) # this is for the box/violinplot to make the x-axis look nicer, maybe there's a more elegant way
new_df$group <- group
new_df$group <- as.factor(new_df$group)
hist(new_df$corr_transcript_length, breaks=100, main = 'correlation AE_VG to transcript per gene over multiple tissues',xlab='spearman correlation')
...
#save(file = 'Rdata/gtex_TL_analysis.Rdata', list= ls())
