...
training <- subset[inTrain,]%>%
  drop_na()
inValidation <- createDataPartition(y = training$log_AE, p=0.60, list =F)
...
test <- subset[-inTrain,]%>%
  drop_na()
rf <- randomForest(log_AE ~ ., data = training, importance=TRUE)
rf
varImpPlot(rf)
...
ggp <- ggplot(df2, aes(test.log_AE, test.pred))+
  ...
  geom_text( x = (-7), y= (-3.5),label = paste('Pearson correlation: ', corr), size=6)


save(file='supervised.Rdata', list=ls())
